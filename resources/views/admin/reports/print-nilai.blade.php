...
<head>
    ...
    <title>Rekap_Nilai_Akademik_Kelas_{{ \Illuminate\Support\Str::slug($selectedKelas) }}_{{ date('Ymd') }}</title>
...
<body class="bg-slate-100 font-sans text-slate-800 p-4 sm:p-8">
    ...
    <div class="max-w-4xl mx-auto mb-6 flex items-center justify-between no-print bg-white p-4 rounded-2xl shadow-md border border-slate-200">
        <div class="flex items-center space-x-3">
            <a href="{{ route('admin.reports.index', ['type' => 'nilai', 'kelas' => $selectedKelas]) }}" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition-all">
                ← Kembali ke Pusat Laporan
            </a>
            <span class="text-xs text-slate-500 font-medium">Kelas: {{ $selectedKelas }} ({{ $rankedData->count() }} Siswa)</span>
        </div>
    ...
    <div class="print-sheet max-w-4xl mx-auto bg-white p-8 rounded-2xl shadow-sm space-y-6">
        ...
        <div class="border-b-2 border-slate-900 pb-3 text-center flex items-center justify-between gap-4">
            ...
            <div class="text-center flex-1">
                ...
                <p class="text-xs text-slate-700 font-bold uppercase tracking-wider mt-1">
                    LEMBAR REKAPITULASI NILAI AKADEMIK & PERANKINGAN SISWA
                </p>
            </div>
            <div class="w-16 h-16 flex items-center justify-center shrink-0">
        ...
        <div class="flex justify-between items-center text-xs font-semibold text-slate-700 bg-slate-50 border border-slate-200 p-2.5 rounded-lg">
            ...
            <div>Rumus Nilai: <span class="font-mono text-slate-900">(Tugas + UH + UTS + UAS) / 4</span></div>
        ...
        <table class="w-full text-left text-xs border-collapse border border-slate-300">
            <thead>
                <tr class="bg-slate-100 text-slate-900 font-bold text-center">
                    <th class="border border-slate-300 p-2 w-12">Rank</th>
                    <th class="border border-slate-300 p-2 w-24">NIS</th>
                    <th class="border border-slate-300 p-2 text-left">Nama Siswa</th>
                    <th class="border border-slate-300 p-2 w-16">Tugas</th>
                    <th class="border border-slate-300 p-2 w-16">UH</th>
                    <th class="border border-slate-300 p-2 w-16">UTS</th>
                    <th class="border border-slate-300 p-2 w-16">UAS</th>
                    <th class="border border-slate-300 p-2 w-20">Nilai Akhir</th>
                    <th class="border border-slate-300 p-2 w-24">Predikat</th>
                </tr>
            </thead>
            <tbody>
                @forelse($rankedData as $idx => $item)
                    @php
                        $na = $item['nilai_akhir'];
                        $predikat = $na >= 90 ? 'A (Sangat Baik)' : ($na >= 80 ? 'B (Baik)' : ($na >= 70 ? 'C (Cukup)' : 'D (Kurang)'));
                    @endphp
                    <tr class="border-b border-slate-200 text-center">
                        ...
                        <td class="border border-slate-300 p-2">{{ $item['nilai_tugas'] }}</td>
                        <td class="border border-slate-300 p-2">{{ $item['nilai_uh'] }}</td>
                        <td class="border border-slate-300 p-2">{{ $item['nilai_uts'] }}</td>
                        <td class="border border-slate-300 p-2">{{ $item['nilai_uas'] }}</td>
                        <td class="border border-slate-300 p-2 font-black text-slate-900 bg-slate-50">{{ $na }}</td>
                        <td class="border border-slate-300 p-2 text-[10px] font-bold">{{ $predikat }}</td>
                    </tr>
                @empty
                    <tr><td colspan="9" class="p-4 text-center text-slate-500">Tidak ada data.</td></tr>
                @endforelse
            </tbody>
        </table>
