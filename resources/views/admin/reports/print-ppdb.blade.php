...
<head>
    ...
    <title>Laporan_Rekapitulasi_PPDB_{{ date('Ymd_His') }}</title>
...
<body class="bg-slate-100 font-sans text-slate-800 p-4 sm:p-8">
    ...
    <div class="max-w-6xl mx-auto mb-6 flex items-center justify-between no-print bg-white p-4 rounded-2xl shadow-md border border-slate-200">
        <div class="flex items-center space-x-3">
            <a href="{{ route('admin.reports.index', ['type' => 'ppdb']) }}" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition-all">
                ← Kembali ke Pusat Laporan
            </a>
            <span class="text-xs text-slate-500 font-medium">Total: {{ $registrations->count() }} Pendaftar</span>
        </div>
    ...
    <div class="print-sheet max-w-6xl mx-auto bg-white p-8 rounded-2xl shadow-sm space-y-6">
        ...
        <div class="border-b-2 border-slate-900 pb-3 text-center flex items-center justify-between gap-4">
            ...
            <div class="text-center flex-1">
                ...
                <p class="text-xs text-slate-700 font-bold uppercase tracking-wider mt-1">
                    LAPORAN REKAPITULASI PENDAFTARAN PPDB (PESERTA DIDIK BARU)
                </p>
            </div>
            <div class="w-16 h-16 flex items-center justify-center shrink-0">
        ...
        <div class="grid grid-cols-3 gap-2 text-xs border border-slate-200 p-3 rounded-lg bg-slate-50">
            @foreach($filterDesc as $k => $v)
                <div><span class="font-bold text-slate-500">{{ $k }}:</span> <span class="font-semibold text-slate-800">{{ $v }}</span></div>
            @endforeach
        ...
        <table class="w-full text-left text-xs border-collapse border border-slate-300">
            <thead>
                <tr class="bg-slate-100 text-slate-900 font-bold text-center">
                    ...
                    <th class="border border-slate-300 p-2 w-28">No. Daftar</th>
                    <th class="border border-slate-300 p-2 text-left">Nama Lengkap</th>
                    <th class="border border-slate-300 p-2 w-24">NISN</th>
                    <th class="border border-slate-300 p-2 w-10">L/P</th>
                    <th class="border border-slate-300 p-2 text-left">Sekolah Asal</th>
                    <th class="border border-slate-300 p-2">Jurusan</th>
                    <th class="border border-slate-300 p-2">Status</th>
                    <th class="border border-slate-300 p-2 text-left">No. HP / WA</th>
                </tr>
            </thead>
            <tbody>
                @forelse($registrations as $idx => $p)
                    <tr class="border-b border-slate-200">
                        <td class="border border-slate-300 p-2 text-center">{{ $idx + 1 }}</td>
                        <td class="border border-slate-300 p-2 text-center font-mono font-bold">{{ $p->no_pendaftaran }}</td>
                        <td class="border border-slate-300 p-2 font-bold">{{ $p->nama_lengkap }}</td>
                        <td class="border border-slate-300 p-2 text-center font-mono">{{ $p->nisn ?? '-' }}</td>
                        <td class="border border-slate-300 p-2 text-center">{{ $p->jenis_kelamin }}</td>
                        <td class="border border-slate-300 p-2">{{ $p->sekolah_asal ?? '-' }}</td>
                        <td class="border border-slate-300 p-2 text-center font-semibold">{{ $p->jurusan ?? '-' }}</td>
                        <td class="border border-slate-300 p-2 text-center font-bold text-[10px] uppercase">
                            {{ $p->status }}
                        </td>
                        <td class="border border-slate-300 p-2 font-mono">{{ $p->no_hp ?? '-' }}</td>
                    </tr>
                @empty
                    <tr><td colspan="9" class="p-4 text-center text-slate-500">Tidak ada data pendaftaran PPDB.</td></tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="pt-8 flex justify-between items-start text-xs">
            ...
            <div class="text-center w-64">
                ...
                <p class="font-bold">Ketua Panitia PPDB</p>
                <div class="h-16"></div>
                <p class="font-bold underline">_________________________</p>
                <p class="text-slate-500">NIP. -</p>
            </div>
