...
<head>
    ...
    <title>Laporan_Audit_Log_Aktivitas_{{ date('Ymd_His') }}</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <style>
        @page { size: A4 portrait; margin: 10mm; }
        @media print {
            .no-print { display: none !important; }
            html, body { background: #fff !important; color: #000 !important; font-size: 9pt !important; padding: 0 !important; margin: 0 !important; }
...
<body class="bg-slate-100 font-sans text-slate-800 p-4 sm:p-8">
    ...
    <div class="max-w-4xl mx-auto mb-6 flex items-center justify-between no-print bg-white p-4 rounded-2xl shadow-md border border-slate-200">
        <div class="flex items-center space-x-3">
            <a href="{{ route('admin.reports.index', ['type' => 'activity']) }}" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition-all">
                ← Kembali ke Pusat Laporan
            </a>
            <span class="text-xs text-slate-500 font-medium">Total: {{ $logs->count() }} Baris Log</span>
        </div>
    ...
    <div class="print-sheet max-w-4xl mx-auto bg-white p-8 rounded-2xl shadow-sm space-y-6">
        ...
        <div class="border-b-2 border-slate-900 pb-3 text-center flex items-center justify-between gap-4">
            ...
            <div class="text-center flex-1">
                ...
                <p class="text-xs text-slate-700 font-bold uppercase tracking-wider mt-1">
                    LAPORAN AUDIT TRAIL & LOG AKTIVITAS SISTEM
                </p>
            </div>
            <div class="w-16 h-16 flex items-center justify-center shrink-0">
        ...
        {{-- Meta info --}}
        <div class="flex justify-between items-center text-xs font-semibold text-slate-700 bg-slate-50 border border-slate-200 p-2.5 rounded-lg">
            <div>Dokumen: <span class="font-bold text-slate-900">Audit Trail Keamanan Sistem</span></div>
            <div>Tanggal Cetak: {{ \Carbon\Carbon::now()->translatedFormat('d F Y H:i:s') }}</div>
        </div>
        ...
        <table class="w-full text-left text-xs border-collapse border border-slate-300">
            <thead>
                <tr class="bg-slate-100 text-slate-900 font-bold text-center">
                    ...
                    <th class="border border-slate-300 p-2 w-32">Waktu</th>
                    <th class="border border-slate-300 p-2 w-36 text-left">Pengguna</th>
                    <th class="border border-slate-300 p-2 w-20">Aksi</th>
                    <th class="border border-slate-300 p-2 text-left">Deskripsi Aktivitas</th>
                    <th class="border border-slate-300 p-2 w-28">IP Address</th>
                </tr>
            </thead>
            <tbody>
                @forelse($logs as $idx => $l)
                    <tr class="border-b border-slate-200">
                        ...
                        <td class="border border-slate-300 p-2 font-mono text-[10px] text-center">{{ $l->created_at ? $l->created_at->format('d/m/Y H:i:s') : '-' }}</td>
                        <td class="border border-slate-300 p-2 font-bold">{{ $l->user?->name ?? ($l->user?->username ?? 'System') }}</td>
                        <td class="border border-slate-300 p-2 text-center font-mono font-bold text-[10px] uppercase">{{ $l->action }}</td>
                        <td class="border border-slate-300 p-2">{{ $l->description }}</td>
                        <td class="border border-slate-300 p-2 text-center font-mono text-[10px]">{{ $l->ip_address ?? '-' }}</td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="p-4 text-center text-slate-500">Tidak ada log aktivitas.</td></tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="pt-8 flex justify-between items-start text-xs">
            ...
            <div class="text-center w-64">
                ...
                <p class="font-bold">Administrator Sistem</p>
                <div class="h-16"></div>
