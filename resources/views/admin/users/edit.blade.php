...
<div class="max-w-2xl space-y-5">
    <div class="flex items-center gap-3">
        ...
        <span class="text-sm text-slate-700 font-semibold">Edit User: {{ $user->name }}</span>
    </div>

    <div class="bg-white rounded-[5px] border border-slate-200 shadow-sm p-6">
        <h3 class="text-base font-bold text-slate-800 mb-5">Edit Informasi User</h3>
        <form action="{{ route('admin.users.update', $user->id_user) }}" method="POST" class="space-y-4">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input name="username" required value="{{ old('username', $user->username) }}"
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 text-sm text-slate-800 outline-none transition-all">
