...
<div class="max-w-2xl space-y-5">
    <div class="flex items-center gap-3">
        ...
        <span class="text-sm text-slate-700 font-semibold">Edit Role: {{ $role->name }}</span>
    </div>

    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        <h3 class="text-base font-bold text-slate-800 mb-5">Edit Informasi Role</h3>
        <form action="{{ route('admin.roles.update', $role->id_role) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 text-sm text-slate-800 outline-none transition-all resize-none">{{ old('description', $role->description) }}</textarea>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <label class="flex items-start gap-3 p-3.5 rounded-2xl border border-slate-200 bg-slate-50 hover:bg-amber-50/50 hover:border-amber-300 transition-all cursor-pointer group">
                        <input type="checkbox" name="permissions[]" value="{{ $permission->id_permission }}"
                               {{ $role->permissions->contains($permission->id_permission) ? 'checked' : '' }}
